...
GameProperties configGame(SDL_DisplayMode display, bool def, char* argv[]) {
    ...
    properties.font = "src/fonts/ClearSans-Bold.ttf";
    
    if (properties.cols < 2) properties.cols = 2;
    if (properties.rows < 2) properties.rows = 2;
    ...
    if (DEBUG_ENABLED) {
        ...
        while (x < count) {
            ...
            if (game.cubes[dx][dy].number == 0) {
                if ((rand() % 10) == 0) {
                    game.cubes[dx][dy].number = 4;
                } else {
                    game.cubes[dx][dy].number = 2;
                }
...
bool moveBoard(SDL_Renderer* renderer, Game game, GameProperties properties, int direction) {
    int score = 0;
    bool moved = false;
    ...
    switch (direction) {
        ...
        case 2: { // RIGHT
            for (int y = 0; y < properties.rows; y++) {
                for (int x = properties.cols - 2; x >= 0; x--) {
                    if (y < 0) continue;
                    if (game.cubes[x][y].number == 0) continue;
