...
void drawAnimation(SDL_Renderer* renderer, Game game, GameProperties properties) {
    SDL_Texture* square = getTexture(renderer, "images/square.png");
    ...
    for (int x = 0; x < properties.cols; x++) {
        for (int y = 0; y < properties.rows; y++) {
            ...
            if (cube.offsetX != 0) {
                SDL_Rect rect = {
                    cube.rect.x + (cube.offsetX - cube.rect.x),
                    cube.rect.y,
                    cube.rect.w,
                    cube.rect.h
                };

                SDL_SetTextureColorMod(square, 150, 150, 150);
                SDL_RenderCopy(renderer, square, NULL, &rect);  
                printf("X: %d -> %d\n", cube.rect.x, cube.offsetX);

                //game.cubes[x][y].offsetX += 5;
            }
        }
    }
...
void drawWin(SDL_Renderer* renderer, Game game, GameProperties properties) {
    SDL_Texture* square = getTexture(renderer, "images/fill.png");
